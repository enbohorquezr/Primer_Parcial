#include <stdio.h>
#include "picachu.h"
picachu::picachu(Tile* _tile, Texture* _texturaPicachu, int _posicionX, int _posicionY, int _ancho, int _alto, int _anchoPantalla, int _altoPantalla, int _velocidadPatron) :
	GameObject(_texturaPicachu, _posicionX, _posicionY, _ancho, _alto, _anchoPantalla, _altoPantalla)
{
	texturaAnimacion = new TextureAnimation();
	texturaAnimacion->setTexture(_texturaPicachu);
...
void picachu::setTile(Tile* _tileNuevo) {

	if (tileActual != nullptr) {
		tileActual->setPicachu(nullptr);
	}
...
void picachu::handleEvent(SDL_Event* event)
{
...
bool picachu::tratarDeMover(MoveDirection _direccionNueva)
{
