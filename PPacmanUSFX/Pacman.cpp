...
void Pacman::handleEvent(SDL_Event* event)
{
	if (event->type == SDL_KEYDOWN && event->key.repeat == 0) {
		switch (event->key.keysym.sym)
		{
			// Move up
		case SDLK_UP:
		case SDLK_i: direccionSiguiente = MOVE_UP; break;

			// Move down
		case SDLK_DOWN:
		case SDLK_k: direccionSiguiente = MOVE_DOWN; break;

			// Move left
		case SDLK_LEFT:
		case SDLK_j: direccionSiguiente = MOVE_LEFT; break;

			// Move right
		case SDLK_RIGHT:
		case SDLK_l: direccionSiguiente = MOVE_RIGHT; break;
		}
	}
}
